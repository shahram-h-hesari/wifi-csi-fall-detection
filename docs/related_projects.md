...
| Awesome-RIS-Security | https://github.com/awesome-ris-security/awesome-ris-security.github.io | Curated literature tracker for reconfigurable intelligent surface (RIS) physical-layer attacks and defenses (arXiv 2025). | Physical-layer attack literature reference for threat model expansion. | External reference only; literature aggregator. License: Pending verification. See `third_party/wifi_sensing_security/awesome_ris_security/`. |
| unilateral-csi-entropy | https://github.com/topics/csi-entropy | CSI-based entropy analysis for edge security and cryptographic key generation. Note: no confirmed canonical public repository identified; topic page listed as placeholder. | Cryptographic entropy reference for CSI-layer security discussion. | External reference only; no code copied. **URL pending verification — no confirmed public repository found.** See `third_party/wifi_sensing_security/unilateral_csi_entropy/`. |
...
| Project | Link | Description | Relevance | Status |
|---------|------|-------------|-----------|--------|
| WiFi-CSI-Human-Pose-Detection | https://github.com/euaziel/WiFi-CSI-Human-Pose-Detection | Human pose estimation using WiFi Channel State Information (CSI) and deep learning (GPL-3.0). | Relevant for fall-detection and through-wall sensing contexts; sensing pipeline reference. | External reference only; no code copied. See `third_party/wifi_sensing/wifi_csi_human_pose_detection/`. |
...
| Project | Link | Description | Relevance | Status |
|---------|------|-------------|-----------|--------|
| RuView | https://github.com/ruvnet/RuView | WiFi-based spatial intelligence, vital sign monitoring, and presence detection using ESP32 and deep learning. | Simulation workflow study and planned offline adversarial robustness evaluation. | External reference only. License: Pending verification. |
| CSI-Bench | https://github.com/Jenny-Zhu/CSI-Bench-Real-WiFi-Sensing-Benchmark | Real WiFi sensing benchmark with standardized evaluation protocols across multiple sensing tasks (fall detection, breathing, localization). | Future dataset candidate for real-data validation phase. | External reference only; no data copied. License: Pending verification. See `datasets/future_datasets/README.md`. |
